            ...
            try {
                if (StringUtils.isNotBlank(orkaEndpoint) && orkaCredentialsId != null) {
                    try (OrkaClientProxy clientProxy = this.clientProxyFactory.getOrkaClientProxy(orkaEndpoint,
                            orkaCredentialsId)) {
                        boolean alreadyInUse = clientProxy.getVMs().getVMs().stream()
    ...
    public FormValidation doCheckNode(String node, String orkaEndpoint, String orkaCredentialsId, String vm,
        ...
        try {
            if (StringUtils.isNotBlank(orkaEndpoint) && orkaCredentialsId != null) {
                try (OrkaClientProxy clientProxy = this.clientProxyFactory.getOrkaClientProxy(orkaEndpoint,
                        orkaCredentialsId)) {
                    hasAvailableNodes = clientProxy.getNodes().getNodes().stream()
                            .filter(ProvisioningHelper::canDeployOnNode).anyMatch(n -> true);

                    if (hasAvailableNodes) {
                        requiredCPU = numCPUs;
                        if (!createNewConfig) {
                            requiredCPU = clientProxy.getVMs().getVMs().stream().filter(v -> v.getVMName().equals(vm))
                                    .findFirst().get().getCPUCount();
                        }

                        OrkaNode nodeDetails = clientProxy.getNodes().getNodes().stream()
                                .filter(n -> n.getHostname().equals(node)).findFirst().get();
